...
import { Camera, Check, Coins, CreditCard, Loader2, Shield, Sparkles, User } from "lucide-react";
...
export default function Profile() {
  ...
  const displayName = profile?.full_name || userEmail?.split("@")[0] || user?.email?.split("@")[0] || "User";
  const initial = (profile?.full_name || userEmail || user?.email)?.charAt(0).toUpperCase() ?? "U";
    ...
    <div className="container max-w-4xl mx-auto px-6 py-10">
      ...
      <div className="mb-10 animate-fade-in">
        <div className="flex items-center gap-3 mb-2">
          <div className="p-2 rounded-lg bg-gradient-to-br from-logo-purple/20 to-electric-blue/20 border border-logo-purple/20">
            <User className="w-5 h-5 text-logo-purple" />
          </div>
          <h1 className="text-3xl font-semibold text-foreground tracking-tight font-display">Profile</h1>
        </div>
        <div className="h-0.5 w-16 rounded-full bg-gradient-to-r from-logo-purple to-electric-blue mt-1" />
        <p className="text-muted-foreground mt-3">Manage your account and preferences.</p>
      </div>

      {/* Hero: Avatar + identity */}
      <div
        className="relative rounded-xl border border-white/[0.06] bg-white/[0.02] p-8 mb-8 overflow-hidden animate-fade-in"
      ...
      >
        ...
        <div className="relative flex flex-col sm:flex-row items-center sm:items-start gap-6">
          <button
            ...
            className="relative group shrink-0 transition-transform duration-200 hover:scale-105"
          >
            {/* Gradient ring */}
            ...
            <Avatar className="relative h-28 w-28 rounded-full ring-2 ring-background shadow-lg">
              ...
              <AvatarFallback className="text-3xl bg-white/[0.06] text-foreground font-medium">
                {initial}
              </AvatarFallback>
            </Avatar>
            {/* Status dot */}
          ...
          <div className="flex-1 text-center sm:text-left">
            ...
            <button
              ...
              onClick={handleAvatarClick}
              disabled={isUpdatingAvatar}
              className="mt-3 text-sm text-primary hover:text-primary/80 font-medium transition-colors inline-flex items-center gap-1.5"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Account info */}
        <div
          className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-6 transition-all duration-200 hover:border-white/[0.1] border-l-2 border-l-logo-purple/40 animate-fade-in"
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                className="bg-white/[0.03] border-white/10 focus-visible:ring-primary/30"
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                value={userEmail}
                disabled
                className="opacity-60 cursor-not-allowed bg-white/[0.02]"
              />
            </div>
            <Button
              type="submit"
        ...
        <div className="space-y-6">
          {/* Credits */}
          <div
            className="relative rounded-xl border border-white/[0.06] bg-white/[0.02] p-6 transition-all duration-200 hover:border-white/[0.1] border-l-2 border-l-logo-purple/40 overflow-hidden animate-fade-in"
            style={{ animationDelay: "150ms" }}
          >
            {/* Radial glow behind credits */}
              ...
              <div className="flex items-end justify-between gap-4">
                  ...
                  <p className="text-4xl font-bold tracking-tight">
                    {isCreditsLoading ? (
                      ...
                      <span className="bg-gradient-to-r from-logo-purple to-electric-blue bg-clip-text text-transparent flex items-center gap-2">
                        <Sparkles className="w-6 h-6 text-logo-purple" />
                        Unlimited
                      </span>
                    ) : (
                ...
                {!profile?.is_unlimited && (
                  <Button
                    ...
                    className="rounded-lg shrink-0 bg-gradient-to-r from-logo-purple to-electric-blue text-primary-foreground shadow-button hover:shadow-button-hover hover:opacity-90 transition-all"
                  >
              ...
              {!profile?.is_unlimited && !isCreditsLoading && (
                <div className="mt-5">
                  <div className="h-1.5 w-full rounded-full bg-white/[0.06] overflow-hidden">
                    <div
                      className="h-full rounded-full bg-gradient-to-r from-logo-purple to-electric-blue transition-all duration-700"
                      style={{ width: `${Math.min(100, Math.max(5, ((credits ?? 0) / Math.max(credits ?? 1, 100)) * 100))}%` }}
                    />
                  </div>
                </div>
              )}
          ...
          {!isAdmin && (
            ...
            >
              <div className="flex items-center gap-2.5 mb-1">
                <div className="p-1.5 rounded-md bg-gradient-to-br from-logo-purple/20 to-electric-blue/20">
                  <CreditCard className="w-4 h-4 text-logo-purple" />
                </div>
                <h3 className="text-lg font-semibold text-foreground tracking-tight">Plan</h3>
              </div>
              <p className="text-sm text-muted-foreground mb-6">Your subscription and billing.</p>
              <div className="space-y-4">
                <div className="flex items-center justify-between py-2 border-b border-white/[0.04]">
                  <span className="text-sm text-muted-foreground">Tier</span>
                  <Badge variant="outline" className="font-medium border-logo-purple/30 text-foreground">
                    {formatPlanTier(profile?.subscription_tier ?? "none")}
                  </Badge>
                </div>
                {profile?.subscription_status && profile.subscription_status !== "none" && (
                  <div className="flex items-center justify-between py-2 border-b border-white/[0.04]">
                    <span className="text-sm text-muted-foreground">Status</span>
                    <span className="font-medium capitalize text-foreground inline-flex items-center gap-2">
                      <span className={`w-2 h-2 rounded-full ${getStatusColor(profile.subscription_status)}`} />
                ...
                {profile?.current_period_end && (
                  <div className="flex items-center justify-between py-2 border-b border-white/[0.04]">
                    ...
                    <span className="font-medium text-foreground">{formatDate(profile.current_period_end)}</span>
                  </div>
                )}
                {profile?.is_unlimited && (
                  <div className="pt-1">
                    <span className="text-sm font-medium bg-gradient-to-r from-logo-purple to-electric-blue bg-clip-text text-transparent inline-flex items-center gap-1.5">
                      <Sparkles className="w-3.5 h-3.5 text-logo-purple" />
                      Unlimited credits
                    </span>
                  </div>
                )}
      ...
      <CreditTopUpModal open={topUpOpen} onOpenChange={setTopUpOpen} />
    </div>
  );
}
